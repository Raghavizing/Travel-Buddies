    ...
    function Card(props) {
        return (
            <div className="card shadow col-lg-3 col-md-5 col-10 mx-3 my-2 rounded-2 text-left p-0 listing-card" data-aos="fade-up">
                ...
                <button className="btn listing-btn text-white px-3 rounded-4 mb-4 ms-2 col-4" onClick={() => { setOpenModal(true); setHotelName(props.name); setHotelLocation(props.location) }}>Details  <FontAwesomeIcon icon={faClipboardList} style={{ color: "#ffffff", }} /></button>
            </div>
        )
    }
    return (<div className="mt-5 listing-page">
        <div className="listingTitle col-11 ms-auto my-4">Most Visited <span className="listingTitleUnderline">Destinations.</span></div>
        <div className="row d-flex justify-content-center" >
            {Hotel.map(createCard)}
        </div>
        <HotelModal show={openModal} name={hotelName} location={hotelLocation} closeModal={setOpenModal} />
    </div>)
}
